// Sub-components for TurnSummaryPanel — structured summary sections.
...
export function getHeadlines(entry: TurnSummaryEntry, theme?: ThemePackage): string[] {
  ...
  if (entry.eliminated) headlines.push('Your civilization has been eliminated.');
  return headlines.slice(0, 3);
}

export function HighlightReel({ headlines }: { headlines: string[] }) {
...
export function ResourceDeltas({ deltas, theme }: { deltas: Record<string, number>; theme?: ThemePackage }) {
...
export function CombatSection({ combats, theme }: { combats: CombatResultSummary[]; theme?: ThemePackage }) {
      ...
      {combats.map((cr, i) => {
        ...
        return (
          <p key={i} className="text-sm text-stone-300">
            ...
            Lost {cr.attackerStrengthLost}/{cr.defenderStrengthLost} str.
          </p>
        );
      })}
    </SummarySection>
...
export function NarrativeSection({ lines, phase }: { lines: string[]; phase: string }) {
...
export function categorizeNarrative(lines: string[]): Record<string, string[]> {
...
export function WorldNews({ summary, civId, theme }: { summary: TurnSummary; civId: string; theme?: ThemePackage }) {
...
export const SECTION_ORDER = ['Diplomacy', 'Movement', 'Economy', 'Construction', 'Research', 'Recruitment', 'Events', 'Stability', 'Other'];
